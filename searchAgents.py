...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        #checking all corners if food present or not
        #flag to check food at all corners
        if len(state[1])==0:
            return True
        else:
            return False
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            #intializing the list to return
            x,y = state[0]
            goals = list(state[1])
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
...
def foodHeuristic(state, problem):
    ...
    flag = 0
    for i in food_list:
        k = mazeDistance(position,i, problem.startingGameState)
        if max1 < k:
            max1 = k 
    return max1    
